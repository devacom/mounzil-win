...
class AddLinkWindow(AddLinkWindow_Ui):
    def __init__(self, parent, callback, mounzil_setting, plugin_add_link_dictionary={}):
        ...
        # read values from mounzil_setting
        # connections
        connections = int(
            self.mounzil_setting.value('settings/connections'))
...
# enable change_name_lineEdit if change_name_checkBox is checked.
    ...
    def okButtonPressed(self, download_later, button=None):
        ...
        # write user's new inputs in mounzil_setting for next time :)
    ...
    def closeEvent(self, event):
        self.mounzil_setting.setValue('AddLinkWindow/size', self.size())
        self.mounzil_setting.setValue('AddLinkWindow/position', self.pos())
