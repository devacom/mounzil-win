...
class PluginsDB():
    def __init__(self):
        ...
        plugins_db_path = os.path.join(mounzil_tmp, 'plugins.db')
...
# mounzil main data base contains downloads information
# This class is managing mounzil.db
class mounzilDB():
    def __init__(self):
        # mounzil.db file path
        mounzil_db_path = os.path.join(config_folder, 'mounzil.db')

        # mounzil_db_connection
        self.mounzil_db_connection = sqlite3.connect(mounzil_db_path, check_same_thread=False)

        # turn FOREIGN KEY Support on!
        self.mounzil_db_connection.execute('pragma foreign_keys=ON')

        # mounzil_db_cursor
        self.mounzil_db_cursor = self.mounzil_db_connection.cursor()
    ...
    def createTables(self):
        ...
        self.mounzil_db_cursor.execute("""CREATE TABLE IF NOT EXISTS category_db_table(
                                                                category TEXT PRIMARY KEY,
        ...
        self.mounzil_db_cursor.execute("""CREATE TABLE IF NOT EXISTS download_db_table(
                                                                                    file_name TEXT,
                                                                                    status TEXT,
                                                                                    size TEXT,
        ...
        self.mounzil_db_cursor.execute("""CREATE TABLE IF NOT EXISTS addlink_db_table(
                                                                                ID INTEGER PRIMARY KEY,
                                                                                gid TEXT,
                                                                                out TEXT,
                                                                                start_time TEXT,
        ...
        self.mounzil_db_cursor.execute("""CREATE TABLE IF NOT EXISTS video_finder_db_table(
                                                                                ID INTEGER PRIMARY KEY,
                                                                                video_gid TEXT,
                                                                                audio_gid TEXT,
        ...
        self.mounzil_db_connection.commit()
    ...
    def insertInCategoryTable(self, dict):
        ...
        self.mounzil_db_cursor.execute("""INSERT INTO category_db_table VALUES(
                                                                            :category,
                                                                            :start_time_enable,
                                                                            :start_time,
                                                                            :end_time_enable,
    ...
    # insert in to download_db_table in mounzil.db
    ...
    def insertInDownloadTable(self, list):
        ...
        for dict in list:
            self.mounzil_db_cursor.execute("""INSERT INTO download_db_table VALUES(
                                                                            :file_name,
                                                                            :status,
                                                                            :size,
                                                                            :downloaded_size,
    ...
    # insert in addlink table in mounzil.db
    ...
    def insertInAddLinkTable(self, list):
        ...
        for dict in list:
            ...
            self.mounzil_db_cursor.execute("""INSERT INTO addlink_db_table VALUES(NULL,
                                                                                :gid,
                                                                                :out,
                                                                                :start_time,
                                                                                :end_time,
    ...
    def insertInVideoFinderTable(self, list):
        ...
        for dictionary in list:
            ...
            self.mounzil_db_cursor.execute("""INSERT INTO video_finder_db_table VALUES(NULL,
                                                                                :video_gid,
                                                                                :audio_gid,
                                                                                :video_completed,
                                                                                :audio_completed,
    ...
    def searchGidInDownloadTable(self, gid):
        ...
        self.mounzil_db_cursor.execute("""SELECT * FROM download_db_table WHERE gid = '{}'""".format(str(gid)))
        list = self.mounzil_db_cursor.fetchall()
    ...
    def returnItemsInDownloadTable(self, category=None):
        ...
        if category:
            ...
            self.mounzil_db_cursor.execute("""SELECT * FROM download_db_table""")

        rows = self.mounzil_db_cursor.fetchall()
    ...
    def searchLinkInAddLinkTable(self, link):
        ...
        self.mounzil_db_cursor.execute("""SELECT * FROM addlink_db_table WHERE link = (?)""", (link,))
        list = self.mounzil_db_cursor.fetchall()
    ...
    def searchGidInAddLinkTable(self, gid):
        ...
        self.mounzil_db_cursor.execute("""SELECT * FROM addlink_db_table WHERE gid = '{}'""".format(str(gid)))
        list = self.mounzil_db_cursor.fetchall()
    ...
    def returnItemsInAddLinkTable(self, category=None):
        ...
        if category:
            ...
            self.mounzil_db_cursor.execute("""SELECT * FROM addlink_db_table""")
...
# this method updates download_db_table
    ...
    def updateDownloadTable(self, list):
        ...
        for dict in list:
            ...
            # update data base if value for the keys is not None
            self.mounzil_db_cursor.execute("""UPDATE download_db_table SET   file_name = coalesce(:file_name, file_name),
                                                                                    status = coalesce(:status, status),
...
# this method updates category_db_table
    ...
    def updateCategoryTable(self, list):
        ...
        for dict in list:
            ...
            # update data base if value for the keys is not None
            self.mounzil_db_cursor.execute("""UPDATE category_db_table SET   start_time_enable = coalesce(:start_time_enable, start_time_enable),
...
# this method updates addlink_db_table
    ...
    def updateAddLinkTable(self, list):
        ...
        for dict in list:
            ...
            # update data base if value for the keys is not None
            self.mounzil_db_cursor.execute("""UPDATE addlink_db_table SET out = coalesce(:out, out),
                                                                                start_time = coalesce(:start_time, start_time),
                                                                                end_time = coalesce(:end_time, end_time),
                                                                                link = coalesce(:link, link),
                                                                                ip = coalesce(:ip, ip),
    ...
    def updateVideoFinderTable(self, list):
        ...
        for dictionary in list:
            ...
            if dictionary['video_gid']:
                # update data base if value for the keys is not None
                self.mounzil_db_cursor.execute("""UPDATE video_finder_db_table SET video_completed = coalesce(:video_completed, video_completed),
    ...
    def setDefaultGidInAddlinkTable(self, gid, start_time=False, end_time=False, after_download=False):
        ...
        if start_time:
            self.mounzil_db_cursor.execute("""UPDATE addlink_db_table SET start_time = NULL
                                                                        WHERE gid = '{}' """.format(gid))
        if end_time:
            self.mounzil_db_cursor.execute("""UPDATE addlink_db_table SET end_time = NULL
                                                                        WHERE gid = '{}' """.format(gid))
        if after_download:
            self.mounzil_db_cursor.execute("""UPDATE addlink_db_table SET after_download = NULL
                                                                        WHERE gid = '{}' """.format(gid))
    ...
    def categoriesList(self):
        ...
        self.mounzil_db_cursor.execute("""SELECT category FROM category_db_table ORDER BY ROWID""")
        rows = self.mounzil_db_cursor.fetchall()
    ...
    def setDBTablesToDefaultValue(self):
        ...
        self.mounzil_db_cursor.execute("""UPDATE category_db_table SET start_time_enable = 'no', end_time_enable = 'no',
                                        reverse = 'no', limit_enable = 'no', after_download = 'no'""")

        # change status of download to 'stopped' if status isn't 'complete' or 'error'
        self.mounzil_db_cursor.execute("""UPDATE download_db_table SET status = 'stopped' 
        ...
        self.mounzil_db_cursor.execute("""UPDATE video_finder_db_table SET checking = 'no'""")
    ...
    def findActiveDownloads(self, category=None):
        ...
        if category:
            self.mounzil_db_cursor.execute("""SELECT gid FROM download_db_table WHERE (category = '{}') AND (status = 'downloading' OR status = 'waiting' 
                                            OR status = 'scheduled' OR status = 'paused')""".format(str(category)))
        else:
            self.mounzil_db_cursor.execute("""SELECT gid FROM download_db_table WHERE (status = 'downloading' OR status = 'waiting' 
                                            OR status = 'scheduled' OR status = 'paused')""")

        # create a list for returning answer
        result = self.mounzil_db_cursor.fetchall()
        gid_list = []
...
# this method returns items with 'paused' status.
    def returnPausedItems(self):
        ...
        # find download items is download_db_table with status = "downloading" or "waiting" or paused or scheduled
        self.mounzil_db_cursor.execute("""SELECT gid FROM download_db_table WHERE (status = 'paused')""")
    ...
    def returnVideoFinderGids(self):
        ...
        self.mounzil_db_cursor.execute("""SELECT video_gid, audio_gid FROM video_finder_db_table""")
...
# this method deletes all items in data_base
    ...
    def resetDataBase(self):
        ...
        self.mounzil_db_cursor.execute("""DELETE FROM download_db_table""")
        self.mounzil_db_cursor.execute("""DELETE FROM addlink_db_table""")
...
# This method deletes a download item from download_db_table
    def deleteItemInDownloadTable(self, gid, category):
        ...
        self.mounzil_db_cursor.execute("""DELETE FROM download_db_table WHERE gid = '{}'""".format(str(gid)))
...
# https://en.wikipedia.org/wiki/Orders_of_magnitude_(data)
    def correctDataBase(self):
        ...
        for units in [['KB', 'KiB'], ['MB', 'MiB'], ['GB', 'GiB']]:
            ...
            self.mounzil_db_cursor.execute("""UPDATE download_db_table 
    ...
    def closeConnections(self):
        ...
        self.mounzil_db_cursor.close()
        self.mounzil_db_connection.close()
