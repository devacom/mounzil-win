...
from mounzil.scripts.osCommands import moveFile, makeTempDownloadDir
from mounzil.scripts.useful_tools import freeSpace, humanReadableSize
...
# get port from mounzil_setting
port = int(mounzil_setting.value('settings/rpc-port'))

# get aria2_path
aria2_path = mounzil_setting.value('settings/aria2_path')
...
# check aria2 release version . mounzil uses this function to
...
def downloadAria(gid, parent):
    ...
    add_link_dictionary = parent.mounzil_db.searchGidInAddLinkTable(gid)
...
# update status and last_try_date in data_base
    ...
    parent.mounzil_db.updateDownloadTable([dict])
    ...
    if start_time_status == "scheduled":
        ...
        parent.mounzil_db.setDefaultGidInAddlinkTable(gid, start_time=True)

    # Find download_path_temp from mounzil_setting
    ...
    download_path_temp = mounzil_setting.value('settings/download_path_temp')
    ...
    if start_time_status != 'stopped':
        ...
        aria_dict = {
            'gid': gid,
            'max-tries': str(mounzil_setting.value('settings/max-tries')),
            'retry-wait': int(mounzil_setting.value('settings/retry-wait')),
            'timeout': int(mounzil_setting.value('settings/timeout')),
            'header': header_list,
            'out': out,
        ...
        if str(mounzil_setting.value('settings/dont-check-certificate')) == 'yes':
...
# if download has completed , then move file to the download folder
    if (converted_info_dict['status'] == "complete"):
        ...
        # if user specified download_path is equal to mounzil_setting download_path,
        # then subfolder must added to download path.
        if mounzil_setting.value('settings/download_path') == download_path:
            download_path = findDownloadPath(
                file_name, download_path, mounzil_setting.value('settings/subfolder'))
        ...
        video_finder_dictionary = parent.mounzil_db.searchGidInVideoFinderTable(gid)
        ...
        parent.mounzil_db.updateAddLinkTable([add_link_dictionary])
    ...
    if free_space != None and file_size != None:
        ...
        if free_space >= file_size:
            ...
            if not(move_answer):
                ...
                logger.sendToLog('mounzil can not move file', "ERROR")
                file_path = path
...
def downloadStop(gid, parent):
    ...
    dict = parent.mounzil_db.searchGidInDownloadTable(gid)
    status = dict['status']
    ...
    if status != 'complete':
        ...
        parent.mounzil_db.setDefaultGidInAddlinkTable(gid,
...
def endTime(end_time, gid, parent):
    ...
    if answer != 'end':
        ...
        parent.mounzil_db.setDefaultGidInAddlinkTable(gid, end_time=True)
