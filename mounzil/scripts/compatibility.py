...
from munzil.scripts.useful_tools import determineConfigFolder
from munzil.scripts.osCommands import remove, removeDir
from munzil.scripts.data_base import munzilDB
from munzil.scripts.newopen import readList
import time
import ast
import os
...
def compatibility():
    ...
    for category in category_list:
        ...
        if category == 'All Downloads' or category == 'Single Downloads':
            munzil_db.updateCategoryTable([category_dict])
        else:
            munzil_db.insertInCategoryTable(category_dict)
    ...
    for line in download_list_file_lines:
        ...
        munzil_db.insertInDownloadTable([dict])
        munzil_db.insertInAddLinkTable([add_link_dictionary])

    # close connections
    munzil_db.closeConnections()
