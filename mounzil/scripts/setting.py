...
from mounzil.gui.setting_ui import Setting_Ui, KeyCapturingWindow_Ui
from mounzil.scripts.useful_tools import returnDefaultSettings
from mounzil.scripts import osCommands
from mounzil.scripts import startup
import platform
import copy
import sys
import os
...
class KeyCapturingWindow(KeyCapturingWindow_Ui):
    def __init__(self, callback, mounzil_setting):
...
class PreferencesWindow(Setting_Ui):
    def __init__(self, parent, mounzil_setting):
        super().__init__(mounzil_setting)
        self.mounzil_setting = mounzil_setting
        self.parent = parent
        self.grandparent = parent.mounzil_main

        self.mounzil_setting.beginGroup('settings')

        # initialization
        self.tries_spinBox.setValue(
            int(self.mounzil_setting.value('max-tries')))
        self.wait_spinBox.setValue(
            int(self.mounzil_setting.value('retry-wait')))
        self.time_out_spinBox.setValue(
            int(self.mounzil_setting.value('timeout')))
        self.connections_spinBox.setValue(
            int(self.mounzil_setting.value('connections')))
        self.rpc_port_spinbox.setValue(
            int(self.mounzil_setting.value('rpc-port')))

        if str(self.mounzil_setting.value('dont-check-certificate')) == 'yes':
        ...
        # wait_queue
        wait_queue_list = self.mounzil_setting.value('wait-queue')
        try:
        ...
        aria2_path = self.mounzil_setting.value('settings/aria2_path')
        ...
        self.download_folder_lineEdit.setText(
            str(self.mounzil_setting.value('download_path')))
        self.temp_download_lineEdit.setText(
            str(self.mounzil_setting.value('download_path_temp')))

        # subfolder
        if str(self.mounzil_setting.value('subfolder')) == 'yes':
        ...
        self.volume_label.setText(
            'Volume : ' + str(self.mounzil_setting.value('sound-volume')))
        self.volume_slider.setValue(
            int(self.mounzil_setting.value('sound-volume')))
        ...
        current_style_index = self.style_comboBox.findText(
            str(self.mounzil_setting.value('style')))
        ...
        current_locale = self.lang_comboBox.findData(
            str(self.mounzil_setting.value('locale')))
        self.lang_comboBox.setCurrentIndex(current_locale)

        self.current_icon = self.mounzil_setting.value('icons')
        ...
        current_icons_size_index = self.icons_size_comboBox.findText(
            str(self.mounzil_setting.value('toolbar_icon_size')))
        ...
        # slider
        self.volume_slider.valueChanged.connect(self.sliderChanged)

        # start_mounzil_if_browser_executed_checkBox
        if str(self.mounzil_setting.value('browser-mounzil')) == 'yes':
            self.start_mounzil_if_browser_executed_checkBox.setChecked(True)
        else:
            self.start_mounzil_if_browser_executed_checkBox.setChecked(False)

        # hide window
        if str(self.mounzil_setting.value('hide-window')) == 'yes':
            self.hide_window_checkBox.setChecked(True)
        else:
            self.hide_window_checkBox.setChecked(False)

        # tray icon
        if str(self.mounzil_setting.value('tray-icon')) == 'yes':
            self.enable_system_tray_checkBox.setChecked(True)
        else:
            self.enable_notifications_checkBox.setChecked(False)

        # show_menubar
        if str(self.mounzil_setting.value('show-menubar')) == 'yes':
        ...
        # show_sidepanel
        if str(self.mounzil_setting.value('show-sidepanel')) == 'yes':
            self.show_sidepanel_checkbox.setChecked(True)
        else:
            self.show_sidepanel_checkbox.setChecked(False)

        # show ProgressWindow
        if str(self.mounzil_setting.value('show-progress')) == 'yes':
        ...
        if str(self.mounzil_setting.value('after-dialog')) == 'yes':
            self.after_download_checkBox.setChecked(True)
        else:
            self.after_download_checkBox.setChecked(False)

        # run mounzil at startup checkBox
        if str(self.mounzil_setting.value('startup')) == 'yes':
            self.startup_checkbox.setChecked(True)
        else:
            self.startup_checkbox.setChecked(False)

        # font_checkBox
        if str(self.mounzil_setting.value('custom-font')) == 'yes':
        ...
        try:  # Integer casting may raise exception.
            self.max_links_spinBox.setValue(int(mounzil_setting.value('video_finder/max_links', 3)))
        except:
            pass
        ...
        # saving initial value of self.mounzil_setting in self.first_key_value_dict
        # at the end! in the okPushButtonPressed method, first_key_value_dict will compared with second_key_value_dict.
        # if any thing changed , then a message box notify user about "some changes take effect after restarting mounzil".
        self.first_key_value_dict = {}
        for member in self.mounzil_setting.allKeys():
            self.first_key_value_dict[member] = str(self.mounzil_setting.value(member))
    ...
    def showCaptureKeyboardWindow(self):
        ...
        keyboard_capture_window = KeyCapturingWindow(self.callBack, self.mounzil_setting)
    ...
    def styleComboBoxChanged(self, index=None):
        ...
        if selected_style != 'Fusion':
            ...
            current_color_index = self.color_comboBox.findText(
                str(self.mounzil_setting.value('color-scheme')))
    ...
    def setDarkLightIcon(self, index=None):
        ...
        if dark_theme == True:
            ...
            current_icons_index = self.icon_comboBox.findText(
                str(self.mounzil_setting.value('icons', self.current_icon)))
    ...
    def closeEvent(self, event):
        ...
        self.mounzil_setting.setValue('PreferencesWindow/size', self.size())
    ...
    def tempDownloadPushButtonClicked(self, button):
        ...
        download_path_temp = str(
            self.mounzil_setting.value('settings/download_path_temp'))
        fname = QFileDialog.getExistingDirectory(
    ...
    def sliderChanged(self, horizontalSlider):
        ...
        self.volume_label.setText('Volume : ' + str(self.volume_slider.value()))
    ...
    def defaultsPushButtonPressed(self, button):
        ...
        self.volume_slider.setValue(int(self.setting_dict['sound-volume']))
    ...
    def okPushButtonPressed(self, button):
        ...
        # change aria2_path
        if self.aria2_path_checkBox.isChecked():
            self.mounzil_setting.setValue('settings/aria2_path', str(self.aria2_path_lineEdit.text()))

        # don't check certificate
        if self.dont_check_certificate_checkBox.isChecked():
            self.mounzil_setting.setValue('dont-check-certificate', 'yes')
        else:
            self.mounzil_setting.setValue('dont-check-certificate', 'no')
        ...
        self.mounzil_setting.setValue('icons', icons)
        ...
        self.mounzil_setting.setValue('toolbar_icon_size', icons_size)
        ...
        self.mounzil_setting.setValue('style', style)

        # language
        locale = str(self.lang_comboBox.itemData(self.lang_comboBox.currentIndex()))
        self.mounzil_setting.setValue('locale', locale)

        # color_scheme
        color_scheme = self.color_comboBox.currentText()
        self.mounzil_setting.setValue('color-scheme', color_scheme)
        ...
        self.mounzil_setting.setValue('font', font)

        font_size = self.font_size_spinBox.value()
        self.mounzil_setting.setValue('font-size', font_size)
        ...
        self.mounzil_setting.setValue('custom-font', custom_font)

        # if user select qt notification  >> enable_system_tray icon
        if self.mounzil_setting.value('notification') == 'QT notification':
            self.enable_system_tray_checkBox.setChecked(True)

        # start_mounzil_if_browser_executed_checkBox
        if self.start_mounzil_if_browser_executed_checkBox.isChecked():
            self.mounzil_setting.setValue('browser-mounzil', 'yes')
        else:
            self.mounzil_setting.setValue('browser-mounzil', 'no')

        # hide_window_checkBox
        if self.hide_window_checkBox.isChecked():
            self.mounzil_setting.setValue('hide-window', 'yes')
        else:
            self.mounzil_setting.setValue('hide-window', 'no')

        # enable_system_tray_checkBox
        if self.enable_system_tray_checkBox.isChecked():
            self.mounzil_setting.setValue('tray-icon', 'yes')
            self.parent.system_tray_icon.show()
            self.parent.minimizeAction.setEnabled(True)
            self.parent.trayAction.setChecked(True)
        else:
            self.mounzil_setting.setValue('tray-icon', 'no')
        ...
        if self.after_download_checkBox.isChecked():
            self.mounzil_setting.setValue('after-dialog', 'yes')
        else:
            self.mounzil_setting.setValue('after-dialog', 'no')

        # show_menubar_checkbox
        if self.show_menubar_checkbox.isChecked():
            self.mounzil_setting.setValue('show-menubar', 'yes')
            self.parent.menubar.show()
            self.parent.toolBar2.hide()
            self.parent.showMenuBarAction.setChecked(True)

        else:
            self.mounzil_setting.setValue('show-menubar', 'no')
        ...
        if self.show_sidepanel_checkbox.isChecked():
            self.mounzil_setting.setValue('show-sidepanel', 'yes')
            self.parent.category_tree_qwidget.show()
        else:
            self.mounzil_setting.setValue('show-sidepanel', 'no')
            self.parent.category_tree_qwidget.hide()

        # show_progress_window_checkbox
        if self.show_progress_window_checkbox.isChecked():
            self.mounzil_setting.setValue('show-progress', 'yes')
        else:
            self.mounzil_setting.setValue('show-progress', 'no')

        if self.startup_checkbox.isChecked():
            self.mounzil_setting.setValue('startup', 'yes')

            if not(startup.checkstartup()):  # checking existence of mounzil in  system's startup

                startup.addstartup()  # adding mounzil to system's startup
        else:
            self.mounzil_setting.setValue('startup', 'no')

            if startup.checkstartup():  # checking existence of mounzil in  system's startup

                startup.removestartup()  # removing mounzil from system's startup

        # keep_awake_checkBox
        if self.keep_awake_checkBox.isChecked():
            self.mounzil_setting.setValue('awake', 'yes')
            self.parent.keep_awake_checkBox.setChecked(True)
        else:
            self.mounzil_setting.setValue('awake', 'no')
        ...
        download_path_temp = self.mounzil_setting.value(
            'download_path_temp')
        download_path = self.mounzil_setting.value('download_path')

        folder_list = [download_path_temp, download_path]

        if self.subfolder_checkBox.isChecked():
            self.mounzil_setting.setValue('subfolder', 'yes')

            for folder in ['Audios', 'Videos', 'Others', 'Documents', 'Compressed']:
                folder_list.append(os.path.join(download_path, folder))

        else:
            self.mounzil_setting.setValue('subfolder', 'no')

        for folder in folder_list:
            osCommands.makeDirs(folder)

        if self.enable_notifications_checkBox.isChecked():
            self.mounzil_setting.setValue('sound', 'yes')
        else:
            self.mounzil_setting.setValue('sound', 'no')

        # columns_tab
        if self.column0_checkBox.isChecked():
            self.mounzil_setting.setValue('column0', 'yes')
        ...
        if self.column12_checkBox.isChecked():
            ...
            self.mounzil_setting.setValue('column12', 'no')
        ...
        # video_finder
        self.mounzil_setting.setValue('video_finder/max_links', self.max_links_spinBox.value())

        # saving value of mounzil_setting in second_key_value_dict.
        self.second_key_value_dict = {}
        for member in self.mounzil_setting.allKeys():
            self.second_key_value_dict[member] = str(self.mounzil_setting.value(member))
        ...
        # if any thing changed that needs restarting, then notify user about "Some changes take effect after restarting mounzil"
        if show_message_box:
            restart_messageBox = QMessageBox()
            restart_messageBox.setText(QCoreApplication.translate(
                "setting_src_ui_tr", '<b><center>Restart mounzil Please!</center></b><br><center>Some changes take effect after restarting mounzil</center>'))
            restart_messageBox.setWindowTitle(QCoreApplication.translate("setting_src_ui_tr", 'Restart mounzil!'))
