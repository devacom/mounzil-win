...
from mounzil.scripts.useful_tools import muxer, freeSpace, determineConfigFolder, osAndDesktopEnvironment
from mounzil.scripts.video_finder_progress import VideoFinderProgressWindow
from mounzil.gui.mainwindow_ui import MainWindow_Ui, QTableWidgetItem
from mounzil.scripts.data_base import PluginsDB, mounzilDB, TempDB
from mounzil.scripts.browser_plugin_queue import BrowserPluginQueue
from mounzil.scripts.after_download import AfterDownloadWindow
from mounzil.scripts.properties import PropertiesWindow
from mounzil.scripts.setting import PreferencesWindow
from mounzil.scripts.progress import ProgressWindow
from mounzil.scripts.play import playNotification
from mounzil.scripts.addlink import AddLinkWindow
from mounzil.scripts.text_queue import TextQueue
from mounzil.scripts.log_window import LogWindow
from mounzil.scripts.update import checkupdate
from mounzil.scripts.shutdown import shutDown
from mounzil.scripts.about import AboutWindow
from mounzil.scripts.bubble import notifySend
from mounzil.scripts import osCommands
from mounzil.scripts import download
from mounzil.scripts import logger
from mounzil.scripts import spider
...
# shutdown_notification = 0 >> mounzil is running
# 1 >> mounzil is ready for closing(closeEvent  is called)
...
# when rpc connection between mounzil and aria is disconnected >>
...
# see mounzil.py file for show_window_file and plugin_ready
plugin_ready = os.path.join(mounzil_tmp, 'mounzil-plugin-ready')

show_window_file = os.path.join(mounzil_tmp, 'show-window')
...
# start aria2 when mounzil starts
...
class StartAria2Thread(QThread):
    ...
    def run(self):
        # aria_startup_answer is None when mounzil starts! and after
        ...
        # 1. no (aria didn't respond) 2. yes(aria is respond) 3.try again(mounzil retry to connecting aria2)
        global aria_startup_answer
        ...
        # if Aria2 doesn't respond to mounzil ,ARIA2RESPONDSIGNAL is
        # emitting no
        if answer == 'did not respond':
...
class CheckDownloadInfoThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            # shutdown_notification = 0 >> mounzil is running
            # 1 >> mounzil is ready for closing(closeEvent called)
            # 2 >> OK, let's close application!
            ...
            while shutdown_notification != 1:
                ...
                try:
                    for gid in active_gid_list:
                        ...
                        if gid not in gid_list:
                            ...
                            if returned_dict:
                                ...
                                # check data_base
                                returned_dict = self.parent.mounzil_db.searchGidInDownloadTable(gid)
                                download_status_list.append(returned_dict)
                    ...
                    # update data base!
                    if update_data_base:
                        self.parent.mounzil_db.updateDownloadTable(download_status_list)
...
# when rpc connection between mounzil and aria is
...
class SpiderThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.parent.mounzil_db.updateDownloadTable([dictionary])
...
class DownloadLink(QThread):
    ...
    def run(self):
        ...
        # if request is not successful then mounzil is checking rpc
...
# mounzil download audio and video separately and the muxing them :)
# VideoFinder do this duty for mounzil.
...
class VideoFinder(QThread):
    ...
    def run(self):
        ...
        # find category
        dictionary = self.parent.mounzil_db.searchGidInDownloadTable(video_gid)
        ...
        if category == 'Single Downloads':
            ...
            add_link_dictionary = self.parent.mounzil_db.searchGidInAddLinkTable(video_gid)
            ...
            if self.video_completed == 'no' and start_time:
                ...
                self.parent.mounzil_db.setDefaultGidInAddlinkTable(audio_gid, start_time=True)

        # update checking status in data base for starting the job!
        self.checking = 'yes'
        self.video_finder_dictionary['checking'] = 'yes'

        self.parent.mounzil_db.updateVideoFinderTable([self.video_finder_dictionary])
        ...
        if category == 'Single Downloads':
            ...
            if shutdown_status == 'wait':
                ...
                # it means user want to mounzil shutdown system after download.
...
class Queue(QThread):
    ...
    def run(self):
        ...
        for counter in range(5):
            ...
            download_table_dict = self.parent.mounzil_db.returnItemsInDownloadTable(self.category)
            category_table_dict = self.parent.mounzil_db.searchCategoryInCategoryTable(self.category)
            ...
            if self.start_time and counter == 0:
                ...
                for gid in gid_list:
                    ...
                    if status != 'complete':
                        ...
                        self.parent.mounzil_db.updateAddLinkTable([add_link_dict])
            ...
            for gid in gid_list:
                ...
                if (gid in self.parent.all_video_finder_gid_list):
                    ...
                    video_finder_dictionary = self.parent.mounzil_db.searchGidInVideoFinderTable(gid)
                ...
                wait_queue_list = self.parent.mounzil_setting.value('settings/wait-queue')
                ...
                while status == 'downloading' or status == 'waiting' or status == 'paused' or status == 'scheduled':
                    ...
                    sleep(1)
                    dictionary = self.parent.mounzil_db.searchGidInDownloadTable(gid)
                ...
                if status == 'stopped':
                    ...
                    for video_finder_gid_list in video_finder_list:
                        ...
                        video_finder_dictionary = self.parent.mounzil_db.searchGidInVideoFinderTable(video_gid)
                        ...
                        if video_finder_dictionary:
                            ...
                            if video_finder_dictionary['video_completed'] == 'no' or video_finder_dictionary['audio_completed'] == 'no':
                                ...
                                self.parent.mounzil_db.updateVideoFinderTable([video_finder_dictionary])
                    ...
                    # show notification
                    notifySend(QCoreApplication.translate("mainwindow_src_ui_tr", "mounzil"),
        ...
        if self.start:
            ...
            if self.after:
                ...
                notifySend(QCoreApplication.translate("mainwindow_src_ui_tr", 'mounzil is shutting down'),
...
# if user is clicking on mounzil icon in menu this tread emits SHOWMAINWINDOWSIGNAL
...
# 2 >> OK, let's close application!
        ...
        while shutdown_notification == 0:
            ...
            # it means , user clicked on mounzil icon and mounzil is
            # still running. see mounzil file for more details.
            ...
            if os.path.isfile(plugin_ready):
                ...
                while plugin_links_checked != True:  # wait for mounzil consideration!
                    sleep(0.5)
...
class MoveThread(QThread):
    ...
    def run(self):
        ...
        for gid in self.gid_list:
            # find download path
            dictionary = self.parent.mounzil_db.searchGidInAddLinkTable(gid)
        ...
        # update data base
        self.parent.mounzil_db.updateAddLinkTable(add_link_dict_list)
...
class MainWindow(MainWindow_Ui):
    def __init__(self, start_in_tray, mounzil_main, mounzil_setting):
        super().__init__(mounzil_setting)
        self.mounzil_setting = mounzil_setting
        self.mounzil_main = mounzil_main
        global icons
        icons = ':/' + \
            str(self.mounzil_setting.value('settings/icons')) + '/'
# add support for other languages
        locale = str(self.mounzil_setting.value('settings/locale'))
        ...
        temp_download_folder = mounzil_setting.value('settings/download_path_temp')
...
# system_tray_icon
        ...
        self.system_tray_icon.setIcon(
            QIcon.fromTheme('mounzil-tray', QIcon(':/mounzil-tray.svg')))
        ...
        if self.mounzil_setting.value('settings/tray-icon') != 'yes' and start_in_tray == False:
        ...
        if self.mounzil_setting.value('settings/show-menubar') == 'yes':
            self.menubar.show()
        ...
        if self.mounzil_setting.value('settings/show-sidepanel') == 'yes':
    ...
    # initializing
        ...
        self.mounzil_db.setDBTablesToDefaultValue()

        # get queues name from data base
        queues_list = self.mounzil_db.categoriesList()
        ...
        download_table_dict = self.mounzil_db.returnItemsInDownloadTable()

        # read gid_list from date base
        category_dict = self.mounzil_db.searchCategoryInCategoryTable('All Downloads')
        ...
        self.all_video_finder_gid_list, self.all_video_finder_video_gid_list, self.all_video_finder_audio_gid_list = self.mounzil_db.returnVideoFinderGids()
        ...
        # keep_awake_checkBox
        if str(self.mounzil_setting.value('settings/awake')) == 'yes':
...
# more details
    def startAriaMessage(self, message):
        ...
        if message == 'yes':
            ...
            self.statusbar.showMessage(
                QCoreApplication.translate("mainwindow_src_ui_tr", "Aria2 didn't respond! be patient! mounzil tries again in 2 seconds!"))
            logger.sendToLog(
                "Aria2 didn't respond! be patient!mounzil tries again in 2 seconds!",
                "WARNING")

        else:
            self.statusbar.showMessage(QCoreApplication.translate("mainwindow_src_ui_tr", 'Error...'))
            notifySend(QCoreApplication.translate("mainwindow_src_ui_tr", 'mounzil can not connect to Aria2'),
                       QCoreApplication.translate("mainwindow_src_ui_tr", 'Check your network & Restart mounzil'),
                       10000, 'critical', parent=self)

            logger.sendToLog('mounzil can not connect to Aria2', 'ERROR')
    ...
    def reconnectAria(self, message):
        ...
        # if message is 'did not respond' then a message(mounzil can not connect to Aria2) shown
        # if message is not 'did not respond' , it means that reconnecting
        # Aria2 was successful.
        if message == 'did not respond':
            self.statusbar.showMessage(QCoreApplication.translate("mainwindow_src_ui_tr", 'Error...'))
            notifySend(QCoreApplication.translate("mainwindow_src_ui_tr", 'mounzil can not connect to Aria2'),
                       QCoreApplication.translate("mainwindow_src_ui_tr", 'Restart mounzil'),
                       10000, 'critical', parent=self)
            ...
            downloading_gid_list = self.mounzil_db.returnDownloadingItems()
            ...
            paused_gid_list = self.mounzil_db.returnPausedItems()

            for gid in paused_gid_list:
                download.downloadStop(gid, self)

            self.statusbar.showMessage(
                QCoreApplication.translate("mainwindow_src_ui_tr", 'mounzil reconnected aria2 successfully'))
            logger.sendToLog('mounzil reconnected aria2 successfully', 'INFO')
...
# read KeepAwakeThread for more information
    def keepAwake(self, add):
        ...
        if self.mounzil_setting.value('settings/awake') == 'yes' and self.ongoing_downloads != 0:
...
# if keep_awake_checkBox toggled by user , this method is called.
    def keepAwakeCheckBoxToggled(self, checkbox):
        if self.keep_awake_checkBox.isChecked():
            self.mounzil_setting.setValue('settings/awake', 'yes')
            self.keep_awake_checkBox.setChecked(True)
        else:
            self.mounzil_setting.setValue('settings/awake', 'no')
            self.keep_awake_checkBox.setChecked(False)
...
#                       'Transfer rate', 'Estimated time left', 'Gid', 'Link', 'First try date', 'Last try date', 'Category']
    ...
    def checkDownloadInfo(self, list):
        ...
        systemtray_tooltip_text = 'mounzil Download Manager'
        ...
        for dict in list:
            ...
            # add download percent to the tooltip text for mounzil system tray icon
            try:
            ...
            if gid in self.all_video_finder_gid_list:
                ...
                video_finder_dictionary = self.mounzil_db.searchGidInVideoFinderTable(gid)
                ...
                if status == 'complete':
                    ...
                    # update data base
                    self.mounzil_db.updateVideoFinderTable([video_finder_dictionary])
            ...
            if gid in self.progress_window_list_dict.keys():
                ...
                # lets do finishing jobs!
                    ...
                    self.mounzil_db.setDefaultGidInAddlinkTable(
                    ...
                    # sync mounzil_setting before checking!
                    self.mounzil_setting.sync()
                    if progress_window.status == "complete" and video_finder_link == False:
                        ...
                        if self.mounzil_setting.value('settings/after-dialog') == 'yes':

                            # show download complete dialog
                            afterdownloadwindow = AfterDownloadWindow(
                                self, dict, self.mounzil_setting)
...
# drag and drop for links
    ...
    # when user requests calls mounzil with browser plugin,
    ...
    def checkPluginCall(self):
        ...
        # get maximum of youtube,... link from mounzil_setting
        max_links = int(self.mounzil_setting.value('settings/video_finder/max_links', 3))
    ...
    # this method creates an addlinkwindow when user calls mounzil with
    ...
    def pluginAddLink(self, add_link_dictionary):
        ...
        addlinkwindow = AddLinkWindow(
            self, self.callBack, self.mounzil_setting, add_link_dictionary)
    ...
    def addLinkButtonPressed(self, button=None):
        addlinkwindow = AddLinkWindow(self, self.callBack, self.mounzil_setting)
        self.addlinkwindows_list.append(addlinkwindow)
        self.addlinkwindows_list[len(self.addlinkwindows_list) - 1].show()

    # callback of AddLinkWindow
    def callBack(self, add_link_dictionary, download_later, category):
        exists = self.mounzil_db.searchLinkInAddLinkTable(add_link_dictionary['link'])
        ...
        self.mounzil_db.insertInDownloadTable([dict])
        self.mounzil_db.insertInAddLinkTable([add_link_dictionary])
        ...
        if selected_row_return != None:
            ...
            if download_status == "paused":
                ...
                if gid in self.all_video_finder_gid_list:
                    ...
                    result_dictionary = self.mounzil_db.searchGidInVideoFinderTable(gid)
    ...
    def propertiesButtonPressed(self, button=None):
        ...
        if selected_row_return != None:
            ...
            propertieswindow = PropertiesWindow(
                self, self.propertiesCallback, gid, self.mounzil_setting, result_dictionary)
    ...
    def progressBarOpen(self, gid):
        ...
        if gid in self.all_video_finder_gid_list:
            ...
            dictionary = self.mounzil_db.searchGidInVideoFinderTable(gid)
            ...
            progress_window = VideoFinderProgressWindow(self, gid_list, self.mounzil_setting)

        else:
            # create an ordinary progress_window
            progress_window = ProgressWindow(
                parent=self, gid=gid, mounzil_setting=self.mounzil_setting)
        ...
        if str(self.mounzil_setting.value('settings/show-progress')) == 'yes':
    ...
    def closeEvent(self, event=None):
        ...
        if str(self.mounzil_setting.value('settings/hide-window')) == 'yes':
    ...
    def closeAction(self, event=None):
        ...
        self.mounzil_setting.setValue('MainWindow/size', self.size())
        self.mounzil_setting.setValue('MainWindow/position', self.pos())

        # save columns size
        self.mounzil_setting.setValue('MainWindow/column0', self.download_table.columnWidth(0))
        self.mounzil_setting.setValue('MainWindow/column1', self.download_table.columnWidth(1))
        self.mounzil_setting.setValue('MainWindow/column2', self.download_table.columnWidth(2))
        self.mounzil_setting.setValue('MainWindow/column3', self.download_table.columnWidth(3))
        self.mounzil_setting.setValue('MainWindow/column4', self.download_table.columnWidth(4))
        self.mounzil_setting.setValue('MainWindow/column5', self.download_table.columnWidth(5))
        self.mounzil_setting.setValue('MainWindow/column6', self.download_table.columnWidth(6))
        self.mounzil_setting.setValue('MainWindow/column7', self.download_table.columnWidth(7))
        self.mounzil_setting.setValue('MainWindow/column10', self.download_table.columnWidth(10))
        self.mounzil_setting.setValue('MainWindow/column11', self.download_table.columnWidth(11))
        self.mounzil_setting.setValue('MainWindow/column12', self.download_table.columnWidth(12))

        # save maximizing situation
        if self.isMaximized():
            self.mounzil_setting.setValue('MainWindow/maximized', 'yes')
        else:
            self.mounzil_setting.setValue('MainWindow/maximized', 'no')

        # sync mounzil_setting
        ...
        # shutdown_notification = 0 >> mounzil running , 1 >> mounzil is
        ...
        for db in self.mounzil_db, self.plugins_db, self.temp_db:
            db.closeConnections()

        QCoreApplication.instance().quit
        logger.sendToLog("mounzil closed!", "INFO")
        sys.exit(0)

    # showTray method shows/hides mounzil's icon in system tray icon
    ...
    def showTray(self, menu=None):
        ...
        # write changes in mounzil_setting
        self.mounzil_setting.setValue('settings/tray-icon', tray_icon)
        self.mounzil_setting.sync()
    ...
    def showMenuBar(self, menu=None):
        ...
        # writing changes to mounzil_setting
        self.mounzil_setting.setValue('settings/show-menubar', show_menubar)
        self.mounzil_setting.sync()
    ...
    def showSidePanel(self, menu=None):
        ...
        # write changes to mounzil_setting
        self.mounzil_setting.setValue(
            'settings/show-sidepanel', show_sidepanel)
        self.mounzil_setting.sync()

    # when user left clicks on mounzil's system tray icon,then
    ...
    def stopAllDownloads(self, menu=None):
        ...
        active_gid_list = self.mounzil_db.findActiveDownloads('Single Downloads')
    ...
    def openAbout(self, menu=None):
        about_window = AboutWindow(self.mounzil_setting)
    ...
    def openDefaultDownloadFolder(self, menu=None):
        # find user's default download folder from mounzil_setting
        self.mounzil_setting.sync()
        download_path = self.mounzil_setting.value('settings/download_path')
    ...
    def openDownloadFolder(self, menu=None):
        ...
        if selected_row_return != None:
            ...
            if download_status == 'complete':
                ...
                # find download path
                dictionary = self.mounzil_db.searchGidInAddLinkTable(gid)
    ...
    def removeSelected2(self):
        ...
        for gid in gid_list:
            ...
            self.mounzil_db.deleteItemInDownloadTable(gid, category)
    ...
    def deleteSelected(self, menu=None):
        ...
        # checking mounzil_setting first!
        # perhaps user was checking "do not show this message again"
        delete_warning_message = self.mounzil_setting.value(
        ...
        if delete_warning_message == 'yes':
            ...
            # if user checks "do not show this message again!", change mounzil_setting!
    ...
    def sortByName2(self):
        ...
        if current_category_tree_text == 'All Downloads':
            downloads_dict = self.mounzil_db.returnItemsInDownloadTable()
        else:
            downloads_dict = self.mounzil_db.returnItemsInDownloadTable(current_category_tree_text)
        ...
        self.mounzil_db.updateCategoryTable([category_dict])
    ...
    def createQueue(self, menu=None):
        ...
        if ok and queue_name != '' and queue_name != 'Single Downloads':
            ...
            answer = self.mounzil_db.searchCategoryInCategoryTable(queue_name)
            ...
            self.mounzil_db.insertInCategoryTable(dict)
    ...
    def pluginQueue(self, list_of_links):
        ...
        plugin_queue_window = BrowserPluginQueue(
            self, list_of_links, self.queueCallback, self.mounzil_setting)
    ...
    def importText(self, menu=None):
        ...
        if os.path.isfile(str(f_path)):
            ...
            text_queue_window = TextQueue(
                self, f_path, self.queueCallback, self.mounzil_setting)
    ...
    def queueCallback(self, add_link_dictionary_list, category):
        ...
        # write information in data_base
        self.mounzil_db.insertInDownloadTable(download_table_dict_list)
        self.mounzil_db.insertInAddLinkTable(add_link_dictionary_list)
    ...
    def categoryTreeSelected2(self, new_selection):
        ...
        if old_selection_index.data():
            ...
            # update data base
            self.mounzil_db.updateCategoryTable([queue_dict])
        ...
        if current_category_tree_text == 'All Downloads':
            ...
            download_table_dict = self.mounzil_db.returnItemsInDownloadTable(current_category_tree_text)

        # get gid_list
        category_dict = self.mounzil_db.searchCategoryInCategoryTable(current_category_tree_text)
        gid_list = category_dict['gid_list']
    ...
    def toolBarAndContextMenuItems(self, category):
        ...
        categories_list = self.mounzil_db.categoriesList()
    ...
    def removeQueue(self, menu=None):
        ...
        # checks mounzil_setting first!
        # perhaps user was checking "do not show this message again"
        remove_warning_message = self.mounzil_setting.value(
        ...
        if current_category_tree_text != 'All Downloads' and current_category_tree_text != 'Single Downloads':
            ...
            self.mounzil_db.deleteCategory(current_category_tree_text)
    ...
    def startQueue(self, menu=None):
        ...
        # update data base
        self.mounzil_db.updateCategoryTable([queue_info_dict])
    ...
    def addToQueue2(self, data):
        ...
        for gid in gid_list:
            ...
            if current_category != new_category:
                ...
                self.mounzil_db.updateDownloadTable([dict])
                self.mounzil_db.setDefaultGidInAddlinkTable(gid, start_time=True, end_time=True, after_download=True)

                # delete item from gid_list in current_category
                current_category_dict = self.mounzil_db.searchCategoryInCategoryTable(current_category)
                ...
                # update category_db_table
                self.mounzil_db.updateCategoryTable([current_category_dict])

                # add item to gid_list of new_category
                # get category_dict from data base
                new_category_dict = self.mounzil_db.searchCategoryInCategoryTable(new_category)
                ...
                # update category_db_table
                self.mounzil_db.updateCategoryTable([new_category_dict])
    ...
    def queuePanelWidget(self, category):
        ...
        queue_info_dict = self.mounzil_db.searchCategoryInCategoryTable(category)
    ...
    def moveDownSelected2(self):
        ...
        # get gid_list from data base
        category_dict = self.mounzil_db.searchCategoryInCategoryTable(
    ...
    def clearDownloadList2(self):
        ...
        gid_list = self.mounzil_db.findActiveDownloads()
    ...
    def showVideoFinderAddLinkWindow(self, input_dict=None, menu=None):
        ...
        if yt_dlp_is_installed and ffmpeg_is_installed:
            ...
            video_finder_addlink_window = VideoFinderAddLink(
                parent=self, receiver_slot=self.videoFinderCallBack, settings=self.mounzil_setting, video_dict=input_dict)
    ...
    def videoFinderCallBack(self, add_link_dictionary_list, download_later, category):
        ...
        for add_link_dictionary in add_link_dictionary_list:
            ...
            # write information in data_base
            self.mounzil_db.insertInDownloadTable([dictionary])
            self.mounzil_db.insertInAddLinkTable([add_link_dictionary])
        ...
        self.mounzil_db.insertInVideoFinderTable([dictionary])
    ...
    def videoFinderCompleted2(self, complete_dictionary):
        ...
        if error_message == 'no error':
            ...
            # find download path
            audio_add_link_dictionary = self.mounzil_db.searchGidInAddLinkTable(complete_dictionary['audio_gid'])
            video_add_link_dictionary = self.mounzil_db.searchGidInAddLinkTable(complete_dictionary['video_gid'])
            ...
            # remove download item from data base
            self.mounzil_db.deleteItemInDownloadTable(
            ...
            # update data base
            self.mounzil_db.updateAddLinkTable([video_add_link_dictionary])

            # get download_table_dict for video_gid
            video_download_table_dict = self.mounzil_db.searchGidInDownloadTable(complete_dictionary['video_gid'])
            ...
            # update data base
            self.mounzil_db.updateDownloadTable([video_download_table_dict])
            ...
            if complete_dictionary['category'] == 'Single Downloads':
                # show download complete dialog
                afterdownloadwindow = AfterDownloadWindow(
                    self, video_download_table_dict, self.mounzil_setting)
