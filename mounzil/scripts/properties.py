...
from mounzil.gui.addlink_ui import AddLinkWindow_Ui
from mounzil.scripts.check_proxy import getProxy
import os


class PropertiesWindow(AddLinkWindow_Ui):
    def __init__(self, parent, callback, gid, mounzil_setting, video_finder_dictionary=None):
...
#frames and checkBoxes
        ...
        self.add_link_dictionary_1 = self.parent.mounzil_db.searchGidInAddLinkTable(self.gid_1)
        self.download_table_dict_1 = self.parent.mounzil_db.searchGidInDownloadTable(self.gid_1)

        if video_finder_dictionary:
            self.add_link_dictionary_2 = self.parent.mounzil_db.searchGidInAddLinkTable(self.gid_2)
            self.download_table_dict_2 = self.parent.mounzil_db.searchGidInDownloadTable(self.gid_2)
...
# get categories name and add them to add_queue_comboBox
        categories_list = self.parent.mounzil_db.categoriesList()
...
# activate frames if checkBoxes checked
    ...
    def okButtonPressed(self, button):
        # write user's new inputs in mounzil_setting for next time if needed
        ...
        if new_category != self.current_category:
            ...
            # update data base
            self.parent.mounzil_db.updateDownloadTable([self.download_table_dict_1])

            if self.video_finder_dictionary:

                # category for audio and video must be same as each other
                self.download_table_dict_2['category'] = new_category
                self.parent.mounzil_db.updateDownloadTable([self.download_table_dict_2])
        ...
        for key in self.add_link_dictionary_1.keys():
            if self.add_link_dictionary_1[key] != self.add_link_dictionary_1_backup[key]:
                ...
                # update data base
                self.parent.mounzil_db.updateAddLinkTable([self.add_link_dictionary_1])
        ...
        if self.video_finder_dictionary:
            for key in self.add_link_dictionary_2.keys():
                if self.add_link_dictionary_2[key] != self.add_link_dictionary_2_backup[key]:
                    ...
                    # update data base
                    self.parent.mounzil_db.updateAddLinkTable([self.add_link_dictionary_2])
            ...
            if self.add_link_dictionary_1['download_path'] != self.add_link_dictionary_1_backup['download_path']:
                ...
                self.parent.mounzil_db.updateVideoFinderTable[dictionary]
    ...
    def closeEvent(self, event):
        ...
        self.mounzil_setting.setValue('PropertiesWindow/size', self.size())
        self.mounzil_setting.setValue(
