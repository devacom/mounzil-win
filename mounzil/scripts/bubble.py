...
if os_type == OS.DARWIN:
    from mounzil.scripts.mac_notification import notifyMac

elif os_type == OS.WINDOWS:
    from mounzil.scripts.windows_notification import Windows_Notification
...
def notifySend(message1, message2, time, sound, parent=None):
    ...
    enable_notification = mounzil_setting.value('settings/notification')
    ...
    if enable_notification == 'QT notification':
        parent.system_tray_icon.showMessage(message1, message2, QIcon.fromTheme('mounzil-tray', QIcon(':/mounzil-tray.svg')), 10000)
    else:
        if os_type in OS.UNIX_LIKE:
            subprocess.Popen(['notify-send', '--icon', 'mounzil',
                              '--app-name', 'mounzil Download Manager',
                              '--expire-time', time,
                              message1, message2],
            ...
            notifyMac("mounzil Download Manager", message1, message2)

        elif os_type == OS.WINDOWS:
            message = Windows_Notification(parent=parent, time=time, text1=message1,
                                           text2=message2, mounzil_setting=mounzil_setting)
            message.show()
