...
from mounzil.constants.Os import OS
import urllib.parse
import subprocess
import platform
import sys
...
def convertToByte(file_size):
    ...
    if file_size[-2:] != ' B':
        ...
        # mounzil uses float type for GiB and TiB
...
def freeSpace(dir):
    ...
    try:
        ...
        if logger_availability:
            logger.sendToLog("mounzil couldn't find free space value:\n" + str(e), "ERROR")
...
def returnDefaultSettings():
    ...
    # mounzil temporary download folder
    if os_type != OS.WINDOWS:
        download_path_temp = home_address + '/.mounzil'
    else:
        download_path_temp = os.path.join(
            home_address, 'AppData', 'Local', 'mounzil')

    # user download folder path
    download_path = os.path.join(home_address, 'Downloads', 'mounzil')
    ...
    default_setting_dict = {'locale': 'en_US', 'toolbar_icon_size': 32, 'wait-queue': [0, 0], 'awake': 'no', 'custom-font': 'no', 'column0': 'yes',
                            ...
                            'tray-icon': 'yes', 'browser-mounzil': 'yes', 'hide-window': 'yes', 'max-tries': 5, 'retry-wait': 0, 'timeout': 60,
...
def muxer(parent, video_finder_dictionary):
    ...
    # find file path
    video_file_dictionary = parent.mounzil_db.searchGidInAddLinkTable(video_finder_dictionary['video_gid'])
    audio_file_dictionary = parent.mounzil_db.searchGidInAddLinkTable(video_finder_dictionary['audio_gid'])
    ...
    video_file_size = parent.mounzil_db.searchGidInDownloadTable(video_finder_dictionary['video_gid'])['size']
    audio_file_size = parent.mounzil_db.searchGidInDownloadTable(video_finder_dictionary['audio_gid'])['size']
    ...
    if free_space:
        if final_file_size > free_space:
            ...
            if parent.mounzil_setting.value('settings/download_path') == final_path:
                if parent.mounzil_setting.value('settings/subfolder') == 'yes':
            ...
            if os_type in OS.UNIX_LIKE:
                ...
                # ffmpeg path in mac
                # ...path/mounzil Download Manager.app/Contents/MacOS/ffmpeg
                cwd = sys.argv[0]
